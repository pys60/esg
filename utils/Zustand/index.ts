...
export const useCategoriesStore = create<
  { categories: Category[]; setCategories: (categories?: Category[]) => void }
>((set) => ({
  categories: [],
  setCategories: (categories) => {
    if (categories) {
      ...
      apiClient.get('categories').then(res => res.json()).then((data:any) => {
        set({ categories: data });
      })
    }
  },
...
export const useContriesStore = create<
  { countries: Country[]; setCountries: () => void }
>((set) => ({
  countries: [],
  setCountries: async () => {
...
export const useUserProjectListStore = create<
  { userProjectList: Project[]; setUserProjectList: () => void }
>((set) => ({
  userProjectList: [],
...
export const useCalcOpenStore = create<
  { calcOpen: boolean; setCalcOpen: (calcOpen: boolean) => void }
>((set) => ({
  calcOpen: false,
...
export const useSupplyOpenModalStore = create<
  { supplyOpenModal: boolean; setSupplyOpenModal: (supplyOpenModal: boolean) => void }
>((set) => ({
...
export const useProfileUpdateStore = create<
  { profileUpdateOpen: boolean; setProfileUpdateOpen: (profileUpdateOpen: boolean) => void }
>((set) => ({
...
export const useSellerProfileUpdateStore = create<
  { sellerProfileUpdateOpen: boolean; setSellerProfileUpdateOpen: (sellerProfileUpdateOpen: boolean) => void }
>((set) => ({
  sellerProfileUpdateOpen: false,
  setSellerProfileUpdateOpen: (sellerProfileUpdateOpen) => set({ sellerProfileUpdateOpen }),
}));
...
>((set) => ({
  step1Data: {
    ...
    dedicatedPerson: {
      ...
      label2: 'Details of the person who may be contacted in case of any queries on the ESG report ',
      value: false,
      type: 'radio',
      required: true,
      isArea: false,
    ...
    supplyCountries: {
      label: 'What countries do you rely on for your supplies or distribution ?',
      value: '',
      type: 'text',
      required: true,
      isAutoFill: true,
    ...
    numOfEmployees: {
      label: 'What is the number of personnel, including permanent and full-time equivalent (FTEs) employees.',
      value: {
        male: '',
        female: '',
      },
    ...
    diffAbledNumOfEmployees: {
      label: 'How many of the above are differently abled personnel, including permanent and full-time equivalent (FTEs) employees.',
      value: {
        male: '',
        female: '',
      },
    ...
    boardComittee: {
      label: "Does the company have a specified committee of the Board/Director responsible for decision making on Sustainability related issues?",
      value: false,
      type: 'radio',
      required: true,
      isAutoFill: false,
...
export const useEnhancedStandardsStore = create<{
  ...
  setEnhancedStandard: (enhancedStandards: {
    name: string;
    value: string;
    selected: boolean;
    file: string
