...
const FileInput = (props: FileInputProps) => {
  ...
  return (
    <div className={`flex flex-col ${props.spaced && 'mt-[40px]'} ${props.className}`}>
      <div className='flex w-full gap-[1rem] mb-[20px] items-center'>
        {(props.label) && <p className={`text-[20px]  ${mode === 'light' ? 'text-grey' : 'text-[#F9F9F9]'
          }`}>{props.label} {props.required && ' *'}</p>}
        {
          file && (
      ...
      <input
        ...
        className={`w-full p-[1rem] h-[48px] border-[1px] ${mode === 'light' ? 'bg-white  border-[rgba(0,0,0,0.5)]' : 'bg-[#121212] text-white  border-[#7C7C7C] placeholder:text-[#7C7C7C]'
