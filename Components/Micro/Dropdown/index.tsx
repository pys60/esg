  ...
  return (
    <div
      ...
      className={`relative select-none flex flex-col ${
        props.spaced && "mt-[40px]"
      } ${props.className}`}
    >
      {props.label && (
        <p
          className={`text-[20px] ${props.labelGap || "mb-[20px]"} ${
      ...
      <div
        className={`w-full p-[1rem] h-[48px] border-[1px] ${
          props.dark
            ...
            : "border-[rgba(0,0,0,0.5)] text-grey"
      ...
      {open && !props.disabled && (
        ...
        >
          ...
          {filteredOptions.map((option) => (
            <div
              key={option.value}
              className={`p-[1rem] hover:bg-[rgba(0,0,0,0.05)] cursor-pointer ${
                option.value === props.selectedValue && "bg-[rgba(0,0,0,0.1)]"
              }`}
              onClick={() => {
          ...
          {filteredOptions.length === 0 && (
            <div className="p-[1rem]">No options found</div>
          )}
        </div>
      )}
