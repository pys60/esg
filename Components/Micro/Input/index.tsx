...
const Input = (props: Props) => {
  ...
  return (
    <div
      className={`flex relative flex-col ${props.spaced && "mt-[40px]"} ${
        props.className
      }`}
    >
      {props.img && (
        <img
          src={props.img}
          alt={props.label}
          className="w-full object-cover h-auto mb-[20px]"
        />
      )}
      <div className="flex items-start justify-between w-full">
        <div className="flex items-center w-full">
          {props.label && !props.diffLabel && props.checkBox && (
            <input
              type="checkbox"
              checked={disabled}
              name={props.name}
              id={props.name}
              className={`w-[20px] h-[20px] accent-blue-500  mr-[1rem] mb-[20px]`}
              onChange={(e) => {
                if (!e.target.checked) {
          ...
          {props.label && !props.diffLabel && (
            <label
              id={props.name}
              className={`text-[20px] ${
                props.checkBox && "w-[90%]"
              } mb-[20px] ${mode === "light" ? "text-grey" : "text-[#F9F9F9]"}`}
            >
        ...
        {loading && (
          <div className="animate-spin">
            <img
              width="30"
              height="30"
              src="https://img.icons8.com/ios-glyphs/30/loading-sign.png"
              alt="loading-sign"
            />
          </div>
        )}
      ...
      <div className="relative w-full">
        {props.isAutoFill && (showButton || isFocused) && (
          <button
            disabled={loading}
            onClick={handleAutoFill}
            className="absolute z-[1]  bottom-[-10vh] right-[0]"
          >
            <img
              src="/Images/SVG/AI.svg"
              alt="AI"
        ...
        {props.isArea ? (
          <textarea
            ...
            name={props.name}
            placeholder={
              (props.placeholder || "Type Here") + (props.required ? "  *" : "")
            }
            value={props.value}
            ...
            className={`w-full disabled:opacity-50 disabled:cursor-not-allowed p-[1rem] ${
              mode === "light"
                ? props.transparent
                  ? "border-[rgba(0,0,0,0.5)] bg-transparent"
                  : "bg-white  border-[rgba(0,0,0,0.5)]"
                : `${
                    props.transparent ? "bg-transparent" : "bg-[#121212]"
                  } text-white  border-[#7C7C7C] placeholder:text-[#7C7C7C]`
            } rounded-[5px] outline-none border-[1px] ${
          ...
          <input
            ...
            className={`w-full disabled:opacity-50 disabled:cursor-not-allowed ${
              props.endIcon ? "pr-[48px]" : "pr-[1rem]"
            }
              p-[1rem] h-[48px] border-[1px] ${
                mode === "light"
                  ? props.transparent
                    ? "border-[rgba(0,0,0,0.5)] bg-transparent"
                    : "bg-white  border-[rgba(0,0,0,0.5)]"
