...
const Chat = ({ open, setOpen }: Props) => {
  const { chatHistory, resetChatHistory, selectedQuestion, setChatHistory, setSelectedQuestion } = useChatHistoryStore()
  ...
  const handleAiClick = async (e: any) => {
    ...
    try {
      ...
      const { data }: any = await axios.post('https://g4ptdzpl66.execute-api.us-east-1.amazonaws.com/ask', {
  ...
  async function queryWithAxios({ file }: FileParam): Promise<any> {
    ...
    try {
      ...
      const response = await axios.post(`${process.env.NEXT_PUBLIC_DASHBOARD_BACKEND_URL}/ai/add-data`, formData, {
        headers: {
  ...
  return (
    <div className='w-full px-[3%]'>
      <form onSubmit={handleAiClick} className='bg-[#001e5be4] flex flex-col justify-between gap-[1rem] relative rounded-xl py-[4vh] h-[75vh] px-[2vw] border border-white'>
        <button onClick={() => setOpen!(!open)} type='button' className='absolute flex items-center justify-center top-[2%] border border-[#001e5be4] rounded-md left-[-2vh] w-[4vh] h-[4vh] bg-white '>
        ...
        <div>
          ...
          {!selectedQuestion && <p className='font-bold text-white truncate text-lg mt-[2vh]'>
            Select Question to Start Chatting...
          </p>}
          {/* marquee */}
          {selectedQuestion && <Marquee autoFill={true} className='w-full mt-[2vh] text-white font-bold text-lg'>
            {selectedQuestion}{"  "}
          </Marquee>}

        </div>
        <div ref={containerRef} className='w-full h-full overflow-y-auto no-scoll-bar'>

          {chatHistory.map((item, index) => (
            <div key={index} className={`flex gap-[1rem] items-start px-[2%] w-full mt-[1rem] ${item.sender === 'user' ? 'justify-end' : 'justify-start'} gap-[1rem]`}>
              {
                item.sender === 'bot' && (
                  <img src='https://zerocarbon.one/favicon.ico' className='w-[1.5rem] h-[1.5rem] rounded-full' />)
              }
              <p className={` ${item.sender === 'user' ? 'text-right text-pri' : 'text-left text-white'
                } w-[80%] text-[16px]`}>{
                  capitalize(item.message)
                }</p>
              {
                item.sender === 'user' && item.message && chatHistory.length > 1 && (
                  <img src='https://i.ibb.co/5Yvtb0w/sdg-wheel.png' className={`w-[1.5rem] h-[1.5rem] ${loading && 'animate-spin'} rounded-full`} />)
              }
            </div>
          ))}
          {
        ...
        <div>
          <div className='relative w-full '>
            ...
            <input disabled={(file ? true : false || loading)} value={value} onChange={(e) => setValue(e.target.value)} className='w-full py-[0.5rem] focus:outline-none border border-white rounded-sm pr-[6vw] px-[3vw]' placeholder={
            ...
            <div className='absolute top-0 right-[0.5rem] flex items-center justify-center h-full'>
              <button type='submit' disabled={file ? false : (!selectedQuestion || loading)} className={`py-[0.2rem] ${!selectedQuestion ? 'cursor-not-allowed' : 'cursor-pointer'
                } px-[1rem] text-white disabled:opacity-50 bg-green-500 rounded-sm`}>
                {loading ? 'Wait...' : file ? 'Upload' : 'Go'}
              </button>
            </div>
          </div>
          <PriButton type='button' disabled={!selectedQuestion || !chatHistory.length} onClick={() => {
