...
const Step2 = ({ title, nextstep, prevstep }: Props) => {
  ...
  const buisnessTripEmi =
    user?.buissnessID?.totalEmissionByCategory?.buisnessTripEmi;
  ...
  const officeEmission =
    user?.buissnessID?.totalEmissionByCategory?.officeEmission;
  const trafficOnWebsiteEmission =
    user?.buissnessID?.totalEmissionByCategory?.trafficOnWebsiteEmi;
  const laptopEmission =
    user?.buissnessID?.totalEmissionByCategory?.laptopEmission;
  const vehicleEmission =
    user?.buissnessID?.totalEmissionByCategory?.vehicleEmission;
  ...
  const scope2 = isNaN(Number(elecEmission) + Number(officeEmission))
    ? 0
    : Number(elecEmission) + Number(officeEmission);
  const scope3 = isNaN(
    Number(empEmission) +
      Number(trafficOnWebsiteEmission) +
      Number(laptopEmission) +
      Number(dataStoreEmi) +
      Number(buisnessTripEmi)
  )
    ? 0
    : Number(empEmission) +
      Number(trafficOnWebsiteEmission) +
  ...
  const selectedGlobalStandards = globalStandards.filter(
    (standard: { selected: boolean; short: string }) => standard.selected
  )[0];
  const enhancedStandard = enhancedStandards.filter(
    (standard: { selected: boolean; file: string }) => standard.selected
  );
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        let step2 =
          ESGQuestions[
            selectedGlobalStandards?.short as keyof typeof ESGQuestions
          ]?.["environmental"];
        // setStep2Data(step2)
        if (
          enhancedStandard.length > 0 &&
          enhancedStandard[0].value !== "None"
        ) {
          enhancedStandard.forEach((item: { file: string }) => {
            step2 = {
              ...step2,
              ...ENHANCED[item.file as keyof typeof ENHANCED]?.environmental,
            };
          });
        }
    ...
    <StepsContainer title={title.title}>
      <form className="w-full h-full">
        ...
        <div className="mt-[40px]">
          <Label checked={true}>{step2Data.organizationScope.label}</Label>
          {/* <Input
            disabled={true}
            checkBox={false}
            transparent={true}
          ...
          <div className="flex items-center gap-[1rem]">
            <PriButton
              ...
              onClick={() => {
                window.open(
                  `https://calculator.zerocarbon.one/?token=${localStorage.getItem(
                    "token"
                  )}&ref=alpha`
                );
              }}
          ...
          <div className="w-full  h-[300px] my-[40px]">
            <div className="relative h-[300px] w-[300px]">
              ...
              <div className="absolute top-0 left-0 flex items-center justify-center w-full h-full">
                <p className="text-[16px] mt-[30px]">{"Scope Data"}</p>
              </div>
            </div>
          </div>
              ...
              <Doughnut
                ...
                data={{
                  ...
                  datasets: [
                    {
                      data: [
                        isNaN(lastYear) ? 0 : lastYear,
                        scope1 + scope2 + scope3 || 1,
                      ],
              ...
              <Input
                ...
                setValue={(e: any) =>
                  setStep2Data({
                    ...step2Data,
                    [key]: {
                      ...step2Data[key as keyof typeof step2Data],
