...
const Step3 = ({ title, nextstep, prevstep }: Props) => {
  ...
  const selectedGlobalStandards = globalStandards.filter(
    (standard: { selected: boolean }) => standard.selected
  )[0];
      ...
      try {
        ...
        if (
          ...
          enhancedStandard.forEach((item: { file: string }) => {
            step2 = {
              ...step2,
              ...ENHANCED[item.file as keyof typeof ENHANCED]?.social,
            };
          });
        }
  ...
  useEffect(() => {
    const step2 =
      ESGQuestions[selectedGlobalStandards?.short as keyof typeof ESGQuestions]
        .social;
    setStep3Data(step2);
          ...
          if (step3Data[key as keyof typeof step3Data].type === "file") {
            return (
              <UploadInput
                ...
                spaced={true}
                urls={
                  step3Data[key as keyof typeof step3Data].url
                    ? [step3Data[key as keyof typeof step3Data].url!]
                    : []
                }
