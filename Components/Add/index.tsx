...
import { useEnhancedStandardsStore, useGlobalStandardsStore, useUserStore } from '@/components/utils/Zustand/store';
...
import CountryDropdown from '../Micro/CountryDropdown';
import PriButton from '../Micro/PriButton';
...
const settings: Settings = {
  ...
  pauseOnHover: true,
  arrows: true,
  nextArrow: <button><MdOutlineNavigateNext className='h-[20px] w-[20px]' /></button>,
  prevArrow: <button><MdOutlineNavigateBefore className='h-[20px] w-[20px]' /></button>,
  responsive: [
    {
      breakpoint: 1024,
      settings: {
...
const Step0 = ({ nextstep }: Props) => {
  ...
  const handleSelectStandard = (name: string) => {
    const newStandards = globalStandards.map(item => {
      if (item.name === name) {
        return {
          ...item,
  ...
  const handleSelectEnhancedStandard = (value: string) => {
    if (value === 'None') {
      const newStandards = enhancedStandards.map(item => {
        if (item.value !== 'None') {
          return {
            ...item,
            selected: false
  ...
  const handleSave = async (type?: string) => {
    ...
    const selectedGlobalStandards = globalStandards.filter(item => item.selected)
  ...
  return (
    <StepsContainer
      prefixIcon={<GrHomeRounded onClick={() => {
        router.push('/buyer')
      }} className='h-[20px] cursor-pointer w-[20px]' />}
      bgcolor='bg-lightPri' pt='2%' noBorder={true} title={
        currentStep === 0 ? 'Create ESG Report' : 'Enhance Standards adopted for a Robust ESG Framework'
      }>
      {currentStep === 0 && <div className='w-full py-[5vh] px-[3vw] '>
        <CountryDropdown
          noLabel={true}
        ...
        <div className='w-full h-[50vh] mt-[5vh]'>
          ...
          <div className='absolute z-[2] top-[30vh] left-0 w-full justify-center gap-[1rem] esg px-[5vw] mt-[5vh]'>
            <Slider {...settings}>
              {
                globalStandards.map((item, index) => {
                  return (
                    <div onClick={() => handleSelectStandard(item.name)} className='w-[15vw] bg-white p-[1vw] py-[3vh] flex flex-col items-center justify-center border border-[#386cd1] rounded-2xl' key={item.name}>
                      <div className='h-[40px] w-[40px] bg-[#386cd1] rounded-full flex items-center overflow-hidden justify-center'>
                        <Image src={`/Images/asset/${item.short}.png`} alt={item.name} className='object-cover' width={40} height={40} />
                      </div>
                      ...
                      <div className='h-[40px] mt-[6vh] w-[40px] border border-black rounded-full flex items-center justify-center'>
                        {
                          item.selected && (
                            <div className='w-[20px] h-[20px] bg-[#386cd1] rounded-full flex items-center justify-center'>
                            </div>
                          )
                        }
                      </div>
      ...
      {currentStep === 1 && <div className='w-full rounded-3xl pt-0  py-[3vh] '>
        <p className='text-[18px] text-grey/50'>
          Transform your ESG reporting with our refined approach that blends your country&apos;s specific regulations with international standards. By adopting our enhanced reporting framework, your ESG report gets elevated stature, designed to engage a broad array of stakeholders with its global perspective and comprehensive insights
        </p>
        <div className='flex flex-wrap gap-[1rem] mt-[2vh]'>
          {
            enhancedStandards.map((item, index) => {
              return (
                <div key={item.name} className='w-[48%] bg-white px-[1%] shadow-lg py-[2%] border border-[#386cd1] rounded-lg flex items-center gap-[1rem]'>
                  <button onClick={() => {
                    ...
                    }} className='text-[15px] transition-all text-[#386cd1] '></TiTick>
                  </button>
                  <p className='text-[16px] w-[90%] font-medium'>
                    <strong>{item.value} {item.name && '| '} </strong> {item.name}
                  </p>
                </div>
              )
            })
      ...
      <div className='h-[10vh] z-10 bg-black w-full gap-[1rem] flex items-center justify-end px-[5%] fixed left-0 bottom-0' >
        <PriButton tooltip='Save and Exit' onClick={() => handleSave()} disabled={loading} className='!px-3 w-fit'>
        ...
        <PriButton tooltip='Previous Step' disabled={
          currentStep === 0
        } onClick={() => {
          if (currentStep === 0) {
            return
        ...
        <PriButton tooltip='Next Step' disabled={loading} onClick={
          () => handleSave('next')
        } className='w-fit !px-3 bg-white'>
